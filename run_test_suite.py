...
from metrics_dashboard import aggregate_level_breakdown
...
def evaluate_citations(
    ...
    if expected_docs:
        citation_valid = recall >= 1.0 and precision > 0
    else:
        citation_valid = len(actual_docs) > 0
...
def ask_via_direct(
    ...
    question: str,
    domain: str,
) -> tuple[dict[str, Any], list[AgentTraceLog]]:
...
def domain_accuracy(cases: list[EvaluatedCase]) -> dict[str, dict[str, float]]:
    ...
    for case in cases:
        primary_domain = case.domain.split(",")[0].strip()
        grouped.setdefault(primary_domain, []).append(case)
...
def build_markdown_report(
    ...
    for label in ["Dễ", "Trung bình", "Khó"]:
...
def _cli() -> None:
    ...
    parser.add_argument(
        "--mode",
        choices=["smoke", "full", "monitoring"],
        default="smoke",
        help="smoke=small sample, full=all cases, monitoring=random sample",
    )
    ...
    parser.add_argument(
        "--runner-mode",
        choices=["api", "direct"],
        default=TEST_RUNNER_MODE,
        help="Execute by calling API /chat or by calling orchestrator directly",
    )
